...
export class ApiService {
  ...
  private static BASE_URL: string = 'http://localhost:8080/api';
  private static ENCRYPTION_KEY = 'shubham-encryp-key';
  ...
  encryptAndSaveToStorage(key: string, value: string): void {
    const encryptedValue = CryptoJS.lib.PasswordBasedCipher.encrypt(
      CryptoJS.algo.AES,
      value,
  ...
  private getFromStorageAndDecrypt(key: string): string | null {
    try {
      ...
      const decryptedValue = CryptoJS.lib.PasswordBasedCipher.decrypt(
        CryptoJS.algo.AES,
        encryptedValue,
        ApiService.ENCRYPTION_KEY
